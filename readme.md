...
read in the data files and format measurement names.
```{r}
...
change the activity names from numbers to descriptive names
merge train and test data files, once a complete data file is created, remove all partial files from memory
...
calculate the mean value for each parameter for each subject performing each activity 
```{r}
